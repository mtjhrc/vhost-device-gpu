...
use vm_memory::{GuestAddress, GuestAddressSpace, GuestMemory, GuestMemoryAtomic, GuestMemoryMmap, VolatileSlice};
use vmm_sys_util::eventfd::EventFd;
use crate::backend::VirtioShmRegion;
use crate::descriptor_utils;
use crate::protocol::GpuResponse::{ErrInvalidResourceId, ErrUnspec, OkCapset, OkCapsetInfo, OkMapInfo, OkNoData, OkResourcePlaneInfo, OkResourceUuid};
use crate::protocol::{GpuResponse, GpuResponsePlaneInfo, VIRTIO_GPU_BLOB_FLAG_CREATE_GUEST_HANDLE, VIRTIO_GPU_BLOB_MEM_HOST3D, VirtioGpuResult};
            ...
            while i < fence_state.descs.len() {
                    ...
                    && fence_state.descs[i].fence_id <= completed_fence.fence_id
                {
                    ...
                    if let Err(e) = vring.add_used(completed_desc.desc_index, completed_desc.len) {
                        log::error!("Failed to add_used descriptor inside Rutabaga fence handler: {e}");
                    }
                    if let Err(e) = vring.signal_used_queue() {
                        log::error!("Failed to signal used queue inside  Rutabaga fence handler: {e}");
                    }
