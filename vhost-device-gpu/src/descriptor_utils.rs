...
impl<'a> Reader<'a> {
    ...
    // TODO: we should get the mem from the chain!
    pub fn new<M>(mem: &'a GuestMemoryMmap, chain: DescriptorChain<M>) -> Result<Reader<'a>>
    where
        M: Deref,
        M::Target: GuestMemory + 'a,
    {
        ...
        let buffers = chain
            ...
            .map(|desc| {
                ...
                mem.get_slice(desc.addr(), desc.len() as usize).map_err(|e| e.into())
            })
...
impl<'a> Writer<'a> {
    ...
    pub fn new<M>(mem: &'a GuestMemoryMmap, chain: DescriptorChain<M>) -> Result<Writer<'a>>
        where M: Deref,
